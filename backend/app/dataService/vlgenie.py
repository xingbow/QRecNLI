...
class VLGenie:
    ...
    def set_vis_type(self, vis):
        ...
        if vis == 'histogram':
            ...
            # The below value is in Pixels and independent of the generated chart dimensions
            # which can be problematic when there are single v/s multiple donut charts.
            # Thus, setting the innerRadius value to 50 for now. Developers will
            # have to override it downstream.
            ...
            # Derive a new "row_number" variable with a sequence of numbers
            # (used as index / counter later on)
    ...
    def create_and_add_column_to_datatable(self, attr):
        ...
        # VL specification for a column of text. Use the row_number
        # as the y_axis encoding to render vertically. Sly!
        column = {
            "width": 150,
            "title": attr,
            "mark": "text",
                ...
                if task["operator"] == 'IN':
                    for attr in task['attributes']:
                        column['transform'].append(
                            {'filter': {"field": attr, "oneOf": task["values"]}})
                elif task["operator"] == 'RANGE':
                    for attr in task['attributes']:
                        column['transform'].append(
                            {"filter": {"field": attr, "range": task["values"]}})
                elif task["operator"] == 'NOT RANGE':
                    for attr in task['attributes']:
                        column['transform'].append(
                            {"filter": {"not": {"field": attr, "range": task["values"]}}})
                else:
                    for attr in task['attributes']:
                        symbol = constants.operator_symbol_mapping[task["operator"]]
                        if helpers.isfloat(task["values"][0]) or helpers.isint(task["values"][0]):
                            column['transform'].append({'filter': 'lower(datum["{}"]) {} {}'.
                                                       format(attr, symbol, task["values"][0])})
                        elif helpers.isdate(task["values"][0]):
                            column['transform'].append({
                                'filter': 'lower(datum["{}"]) {} "{}"'.format(
    ...
    def add_label_attribute_as_tooltip(self, label_attribute):
        # Check if any of the AXES (encodings) have existing aggregations.
        # If not, then add tooltip which is the label
        has_aggregate = False
        for encoding in self.vl_spec['encoding']:
            if 'aggregate' in self.vl_spec['encoding'][encoding] and \
                    self.vl_spec['encoding'][encoding]['aggregate'] is not None:
                has_aggregate = True
                break
